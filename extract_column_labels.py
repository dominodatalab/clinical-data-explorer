...
def main():
    ...
    print("Extracting variable definitions from definitions.html...")
    ...
    print(f"Found {len(combined)} total variable/dataset definitions")
    
    # Consolidate into master lookup
    consolidated = consolidate_results(combined)
    
    print(f"Consolidated into {len(consolidated)} unique column names")
    ...
    print(f"Wrote {simple_output}")
    
    # Print sample of extracted data
    print("\nSample of extracted column labels:")
    print("-" * 60)
    for row in sorted(consolidated, key=lambda x: x['column_name'])[:20]:
        print(f"{row['column_name']:20} -> {row['label']}")
    print("...")
