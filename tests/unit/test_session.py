...
def test_mcp_get_applies_default_timeout_and_session_header(monkeypatch):
    ...
    monkeypatch.setattr(backend_session.httpclient.requests, "get", fake_get)
...
def test_mcp_post_allows_explicit_timeout_override(monkeypatch):
    ...
    monkeypatch.setattr(config, "MCP_SERVER_URL", "http://mcp.example")
    monkeypatch.setattr(config, "MCP_REQUEST_TIMEOUT_SECONDS", 42)
    monkeypatch.setattr(backend_session, "get_passthrough_token", lambda: "token-2")
    monkeypatch.setattr(backend_session.httpclient.requests, "post", fake_post)
    ...
    assert captured == [
        (
            ...
            {
                "headers": {"X-Trace-Id": "trace-1", "Authorization": "Bearer token-2"},
                "json": {"page": 1},
                "timeout": 7,
            },
        )
