...
DATA_FILE_SIZE_LIMIT = os.environ.get('DATA_FILE_SIZE_LIMIT_B', 500 * ONE_MB)
RAM_BUFFER = os.environ.get('SERVICE_RAM_BUFFER_B', 5 * ONE_MB)
...
def enforce(file_name: str, file_size: int):
    ...
    if file_size > DATA_FILE_SIZE_LIMIT:
        raise HTTPException(
            status_code=413,
            detail=f'{file_name} must be less than or equal to {DATA_FILE_SIZE_LIMIT} to be processable'
        )

    # this is the estimated size that the dataframe will be when it's created
    estimated_df_size_b = metadata.fileSize*5
